...
카드 밖 위에 연락처 한 줄, 카드 밖 아래에 기술 스택 레일이 있습니다.
`.header.flex` 안에서 `flex: 0 0 100%` 인 형제 둘(`.page-contact`,
`.page-rail`)이 각각 한 줄로 접히는 구조입니다. 폭은 카드(`.container`)의
규칙을 그대로 따라갑니다 — 어긋나면 카드 위아래로 삐져나온 띠처럼 보입니다.

카드 밖 글자는 `--surface` 가 아니라 `--page-bg` 위에 있습니다. 라이트의
붉은 바탕(`#D24D57`)은 흰 글자와 **4.24:1** 이라 본문 기준(4.5:1)에 못
미쳐서, 전용 토큰 `--page-text` 가 라이트에서는 거의 검정, 다크에서는 밝은
회색으로 방향을 뒤집습니다. `contrast.py` 가 이 쌍을 게이트합니다.

**기술 스택 레일**은 같은 목록을 두 벌 내고 한 벌 너비(`-50%`)만큼 밀어
되돌립니다. 그래서 이음매가 보이지 않습니다. 간격을 flex `gap` 이 아니라
항목의 `margin-right` 로 주는 이유가 여기 있습니다 — `gap` 이면 두 벌 사이에
간격이 한 번 더 들어가 한 벌 너비와 `-50%` 가 어긋나고, 한 바퀴마다 몇
픽셀씩 튑니다.

둘째 벌은 눈속임이라 `aria-hidden` 이고 그 안의 링크는 `tabindex="-1"` 입니다.
초점이 갈 수 있는 요소를 `aria-hidden` 안에 두는 것 자체가 접근성 위반이라
둘을 함께 해야 합니다.

멈춤 장치는 `:hover` 와 `:focus-within` 둘 다입니다. 세울 방법이 마우스뿐이면
키보드로는 흐르는 링크를 잡을 수 없습니다. 움직임을 줄여 달라고 설정한
방문자에게는 아예 세우고 대신 가로 스크롤을 엽니다. 이 네 가지는 마크업이
아니라 CSS 에 있어서, 검증 스크립트가 컴파일된 `main.css` 를 직접 봅니다.
