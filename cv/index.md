...
한때 이 페이지만 드림플레이 종료가 '2025.04' 로 적혀 있었고 ACT Games 와
퍼플오션이 통째로 빠져 있었다 — 사이트와 이력서를 나란히 보는 채용담당에게는
그 차이가 가장 먼저 보인다. 회사·기간·수치를 고칠 때는 사실표를 먼저 고치고
제출 문서 3종과 이 페이지를 함께 맞춘다.

사실표와 이 페이지는 한 건에서 일부러 어긋나 있다. 2025.10 ~ 2026.03 의 재직
이력 한 건은 본인 요청으로 이 저장소 전체에서 제외했다. 사실표에 있는데 여기
없다고 해서 '빠뜨린 회사' 로 보고 되살리지 말 것. 아래 총 경력 4년 4개월도
그 6개월을 뺀 값이라 사실표 합계와 다른 것이 정상이다. 반대로 그 기간의
작업 가운데 회사명이 붙지 않는 기술(Steam · Jenkins · IAP)은 /about/ 의 기술
스택에 그대로 남겨 두었다 — 근거 없는 항목으로 보고 지우지 말 것.
...
**총 4년 4개월** (2026.09 기준) · Unity 클라이언트 개발
